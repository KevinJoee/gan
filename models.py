...
from .networks import Discriminator, PRNet,DRNet, Depthlayer
...
class Model(BaseModel):
    def __init__(self, config):
        ...
        self.depth_estimator = Depthlayer(config.BASE_CHANNEL_NUM // 2, min_d=config.MIN_D, max_d=config.MAX_D, path=self.gen_weights_path[:-4]+'_'+self.name+'.pth')
        self.net_h2c = PRNet(config.BASE_CHANNEL_NUM //2 , min_beta=config.MIN_BETA, max_beta=config.MAX_BETA, min_d=config.MIN_D, max_d=config.MAX_D, path=self.gen_weights_path[:-4]+'_'+self.name+'.pth', use_dc_A=config.USE_DC_A)
        self.net_c2h = DRNet(config.BASE_CHANNEL_NUM // 2, in_channels=3, out_channels=3, min_beta=config.MIN_BETA, max_beta=config.MAX_BETA)
