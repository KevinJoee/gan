...
class PRNet(BaseNetwork):
    def __init__(self, base_channel_nums, init_weights=True, path=None, min_beta=0.04, max_beta=0.2, min_d=0.3, max_d=5, use_dc_A=0):
        ...
        self.final_conv_beta_1 = conv_block(in_nc=32 +48 +136, out_nc=2*base_channel_nums, kernel_size=1, act_type=None, norm_type=None, use_spectral_norm=use_spectral_norm)
        ...
        self.e1 = SFIB(in_nc=base_channel_nums // 2)
        ...
        self.e2 = SFIB(in_nc=base_channel_nums)
        ...
        self.enc_conv2 = conv_block(in_nc=base_channel_nums, out_nc=2 * base_channel_nums, kernel_size=3, stride=2,
                ...
                #
        self.bottleneck1 = SFIB(in_nc=2 * base_channel_nums)

        self.bottleneck2 = SFIB(in_nc=2 * base_channel_nums)
        self.bottleneck3 = SFIB(in_nc=2 * base_channel_nums)
        self.bottleneck4 = SFIB(in_nc=2 * base_channel_nums)


        self.dec_up2 = nn.UpsamplingBilinear2d(scale_factor=2)
        self.dec_conv2 = conv_block(base_channel_nums * 2, out_nc=base_channel_nums * 1, kernel_size=3, act_type=act_type, norm_type=None, pad_type='reflect', mode=mode, use_spectral_norm=use_spectral_norm)

        self.d1 = SFIB(in_nc=base_channel_nums* 1)
        self.dec_conv_last = conv_block(base_channel_nums * 1, out_nc=base_channel_nums//2, kernel_size=3, pad_type='reflect',
                                        act_type=act_type, norm_type=None, use_spectral_norm=use_spectral_norm)
        self.d2 = SFIB(in_nc=base_channel_nums//2)

        self.dec_conv_last_2 = conv_block(base_channel_nums //2, out_nc=3, kernel_size=3,
                                          pad_type='reflect',
                                          act_type=act_type, norm_type=None, use_spectral_norm=use_spectral_norm)
    ...
    def forward(self, x_0, require_paras=False, times=1.0, use_guided_filter=False):
        ...
        layer_1_beta = F.adaptive_avg_pool2d(x, [1,1]).detach()
        layer_2_beta = F.adaptive_avg_pool2d(x1, [1, 1]).detach()
        layer_3_beta = F.adaptive_avg_pool2d(x2, [1, 1]).detach()
        ...
        #print(d)************************
        dehaze = ((x_0-A)/t + A).clamp(0,1)
        

        if require_paras:
            return dehaze1, d, beta
        else:
            return dehaze1,t
...
class Depthlayer(BaseNetwork):
    def __init__(self, base_channel_nums=48, min_d=0.3, max_d=10, path=None, init_weights=True):
        ...
        self.bottleneck1 = SFIB(in_nc= base_channel_nums)
        self.bottleneck2 = SFIB(in_nc= base_channel_nums)
        self.bottleneck3 = SFIB(in_nc= base_channel_nums)
        
        self.out = conv_block(base_channel_nums * 1, out_nc=3, kernel_size=3,
                                          pad_type='reflect',
                                          act_type=act_type, norm_type=None, use_spectral_norm=use_spectral_norm)
        # self.invert =Invert(img)
...
class SFIB(nn.Module):
    def __init__(self, in_nc):
        ...
        self.frequency_process1 = FreBlock1(in_nc)
        ...
        self.frequency_process2 = FreBlock2(in_nc)
    ...
    def forward(self, x):
        ...
        x_freq = self.frequency_process1(x_freq)+1e-8
        x1 = torch.fft.irfft2(x_freq, s=(H, W), norm='backward')+1e-8
        x_1 = torch.cat([x,x1],1)
        x_1 = self.cat(x_1)
        
        x = self.spatial_process2(x_1)+1e-8
        x_freq = self.frequency_process2(x_1)+1e-8
        x2 = (torch.fft.irfft2(x_freq, s=(H, W), norm='backward'))
        xcat = torch.cat([x,x2],1)
        x_out = self.cat(xcat)
